...
def wae_mcsm(encoder, decoder, score, score_opt, X, training=True, n_energy_opt=1, n_particles=1, lam=10, eps=1e-3, type=0):
    z = encoder(X)
    mcsm_losses, *_ = mcsm_loss_optimized(score, z, n_particles=n_particles, eps=eps, type=type)
    if training:
        score_opt.zero_grad()
...
def wae_mcsm_forward(encoder, decoder, score, score_opt, X, training=True, n_energy_opt=1, n_particles=1, lam=10, eps=1e-3, type=0):
    z = encoder(X)
    mcsm_loss, *_ = mcsm_forward_loss_optimized(score, z, n_particles=n_particles, eps=eps, type=type)
    if training:
        score_opt.zero_grad()
        mcsm_loss.backward()
        score_opt.step()
...
def wae_mcsm_backward(encoder, decoder, score, score_opt, X, training=True, n_energy_opt=1, n_particles=1, lam=10, eps=1e-3, type=0):
    z = encoder(X)
    mcsm_loss, *_ = mcsm_backward_loss_optimized(score, z, n_particles=n_particles, eps=eps, type=type)
    if training:
        score_opt.zero_grad()
        mcsm_loss.backward()
        score_opt.step()
        for i in range(n_energy_opt - 1):
            z = encoder(X)
            mcsm_loss, *_ = mcsm_backward_loss_optimized(score, z, eps=eps, n_particles=n_particles, type=type)
            score_opt.zero_grad()
            mcsm_loss.backward()
            score_opt.step()

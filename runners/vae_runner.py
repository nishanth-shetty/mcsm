...
class VAERunner():
    ...
    def train(self):
        ...
        if self.config.data.dataset == 'CIFAR10':
            ...
            train_indices, test_indices = indices[:int(
                num_items * 0.8)], indices[int(num_items * 0.8):]
        ...
        if self.config.training.algo == 'vae':
            ...
            optimizer = self.get_optimizer(itertools.chain(
            ...
            opt_score = optim.RMSprop(
                score.parameters(), lr=self.config.optim.lr)
            if self.args.resume_training:
                states = torch.load(os.path.join(
                    self.args.log, 'checkpoint.pth'))
            ...
            ) if self.config.training.algo == 'spectral' else SteinScoreEstimator()
            optimizer = self.get_optimizer(itertools.chain(
                imp_encoder.parameters(), decoder.parameters()))
            if self.args.resume_training:
                states = torch.load(os.path.join(
                    self.args.log, 'checkpoint.pth'))
                imp_encoder.load_state_dict(states[0])
        ...
        for _ in range(self.config.training.n_epochs):
            for _, (X, y) in enumerate(dataloader):
                ...
                if self.config.training.algo == 'vae':
                    ...
                    if self.config.training.method == "central":
                        loss, mcsm_loss, *_ = elbo_mcsm(imp_encoder, decoder, score, opt_score, X, recon_type,
                                                  training=True, n_particles=self.config.model.n_particles,
                                                  eps=self.config.training.eps, var_type=self.config.training.var_type)
                ...
                if step % 10 == 0:
                    ...
                    if self.config.data.dataset == 'CELEBA':
                        test_X = test_X + \
                            (torch.rand_like(test_X) - 0.5) / 128.
                    elif self.config.data.dataset == 'MNIST':
                    ...
                    if self.config.training.algo == 'vae':
                        ...
                        with torch.no_grad():
                            test_loss, * \
                                _ = elbo(encoder, decoder, test_X, recon_type)
                            logging.info("loss: {}, test_loss: {}".format(
                                loss.item(), test_loss.item()))
                    elif self.config.training.algo == 'ssm':
                        imp_encoder.eval()
                        test_loss, * \
                            _ = elbo_ssm(imp_encoder, decoder, score,
                    ...
                    tb_logger.add_scalar(
                        'test_loss', test_loss, global_step=step)

                    if self.config.training.algo == 'ssm':
                        tb_logger.add_scalar(
                            'ssm_loss', ssm_loss, global_step=step)
                    if self.config.training.algo == 'mcsm':
                        tb_logger.add_scalar(
                            'mcsm_loss', mcsm_loss, global_step=step)

                if step % 500 == 0:
                    with torch.no_grad():
                        z = torch.randn(
                            100, self.config.model.z_dim, device=X.device)
                        decoder.eval()
                        if self.config.data.dataset == 'CELEBA':
                            samples, _ = decoder(z)
                            ...
                            image_grid = torch.clamp(
                                image_grid / 2. + 0.5, 0.0, 1.0)
                            data_grid = make_grid(X[:100], 10)
                            data_grid = torch.clamp(
                                data_grid / 2. + 0.5, 0.0, 1.0)
                        elif self.config.data.dataset == 'MNIST':
                        ...
                        tb_logger.add_image(
                            'samples', image_grid, global_step=step)
                        tb_logger.add_image(
                            'data', data_grid, global_step=step)
    ...
    def test(self):
        ...
        decoder.eval()
        decoder.load_state_dict(states[1])
        z = torch.randn(100, self.config.model.z_dim,
    ...
    def test_fid(self):
        ...
        if get_data_stats:
            ...
            if not os.path.exists(os.path.join(self.args.run, 'datasets', 'celeba140_fid', 'raw_images')):
                os.makedirs(os.path.join(self.args.run, 'datasets',
                            'celeba140_fid', 'raw_images'))
            ...
            if manual:
                ...
                if not os.path.exists(os.path.join(self.args.log, 'samples', 'raw_images')):
                    os.makedirs(os.path.join(
                        self.args.log, 'samples', 'raw_images'))
                logging.info("Images generated. Saving images")
                for i, image in enumerate(all_samples):
                    save_image(image, os.path.join(self.args.log,
                               'samples', 'raw_images', '{}.png'.format(i)))
                ...
                for iter in range(1, 11):
                    ...
                    if not os.path.exists(os.path.join(self.args.log, 'samples', 'raw_images_{}0k'.format(iter))):
                        os.makedirs(os.path.join(self.args.log,
                                    'samples', 'raw_images_{}0k'.format(iter)))
                    else:
                    ...
                    if not os.path.exists(os.path.join(self.args.log, 'samples', 'statistics_{}0k'.format(iter))):
                        os.makedirs(os.path.join(self.args.log,
                                    'samples', 'statistics_{}0k'.format(iter)))
                    else:
                        shutil.rmtree(os.path.join(
                    ...
                    fid_number = fid.calculate_fid_given_paths([
                        ...
                        os.path.join(self.args.log, 'samples', 'statistics_{}0k'.format(iter), 'celeba_test.npz')], 50, True, 2048)
                    logging.info(
    ...
    def test_ais(self):
        ...
        def recon_energy(X, z):
            if recon_type is 'gaussian':
                ...
                recon = (X - mean_x) ** 2 / (2. * (2 * logstd_x).exp()
                                             ) + np.log(2. * np.pi) / 2. + logstd_x
    ...
    def test_svi(self):
        ...
        for batch, (X, y) in enumerate(test_loader):
            ...
            gaussian = Gaussian4SVI(
                X.shape[0], self.config.model.z_dim).to(self.config.device)
            optimizer = optim.SGD(gaussian.parameters(), lr=0.01, momentum=0.5)
            lr_scheduler = optim.lr_scheduler.MultiStepLR(
                optimizer, milestones=[100, 200], gamma=0.3)
            for i in range(300):
                lr_scheduler.step()
                loss = iwae(gaussian, decoder, X,
                            type=recon_type, k=10, training=True)
                decoder.zero_grad()
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                print(i, loss.item())
            loss = iwae(gaussian, decoder, X,
                        type=recon_type, k=10, training=False)

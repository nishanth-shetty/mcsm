...
def parse_args_and_config():
    ...
    parser.add_argument('--dsm_sigma', type=float,
                        default=0.16, help='Sigma for DSM tuning')
    parser.add_argument('--scalability_dim', type=int,
                        default=10, help='Dimension for scalability testing')
    parser.add_argument('--load_path', type=str, default='',
                        help='Path to state dict for resuming training')
    parser.add_argument('--device_number', type=int, default=0,
                        help='GPU number you want to train on')
    ...
    # add device
    device = torch.device(
        f'cuda:{args.device_number}') if torch.cuda.is_available() else torch.device('cpu')
